...
local function player1Mechanics( player1)
    if(pT) then
    player1:setLinearVelocity(math.sin(math.rad(player1.rotation)) * -myData.player1.speed, math.cos(math.rad(player1.rotation)) * myData.player1.speed)
end
end
...
function scene:show( event )
    ...
    physics.addBody( myData.goal1, "static", {isSensor = true} )
    physics.addBody( myData.goal2, "static", {isSensor = true} )
...
function scene:hide( event )
    ...
    if ( phase == "will" ) then
        resetGameState()
        pT = false
        --sceneGroup:remove( myData.rotateClock )
